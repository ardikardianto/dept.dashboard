...
function FloatingBottomNav({ active, setActive, onLogout }) {
  return <nav className="mobile-bottom-nav fixed inset-x-0 bottom-4 z-40 px-3 sm:bottom-6 sm:px-6"><div className="mx-auto flex max-w-5xl items-center gap-2 overflow-x-auto rounded-[1.75rem] border border-white/30 bg-[linear-gradient(180deg,rgba(255,255,255,0.16),rgba(255,255,255,0.05)),rgba(10,18,29,0.88)] p-2 shadow-[0_22px_70px_rgba(12,23,36,0.42),inset_0_1px_0_rgba(255,255,255,0.22)] backdrop-blur-2xl backdrop-saturate-200"><div className="hidden shrink-0 items-center gap-3 border-r border-white/15 px-3 pr-4 md:flex"><div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#005baa]/85 text-[#ffd23f] shadow-[inset_0_1px_0_rgba(255,255,255,0.2)]"><Icons.graduation className="h-5 w-5" /></div><div><p className="text-sm font-black text-white">{department.name}</p><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#ffd23f]">{department.subtitle}</p></div></div>{nav.map((item) => { const Icon = item.icon; const selected = active === item.id; return <button key={item.id} title={item.label} onClick={() => setActive(item.id)} className={`flex min-w-16 shrink-0 flex-col items-center justify-center gap-1 rounded-2xl px-3 py-2 text-[11px] font-black transition sm:min-w-24 sm:flex-row sm:px-4 sm:text-sm ${selected ? "bg-[#ffd23f] text-[#102f52] shadow-[0_10px_26px_rgba(255,210,63,0.24),inset_0_1px_0_rgba(255,255,255,0.42)]" : "bg-white/[0.07] text-white hover:bg-white/14 hover:text-white"}`}><Icon className="h-5 w-5" /><span>{item.label}</span></button>; })}<button title="Logout" onClick={onLogout} className="ml-auto flex min-w-16 shrink-0 flex-col items-center justify-center gap-1 rounded-2xl border border-white/20 bg-white/[0.07] px-3 py-2 text-[11px] font-black text-white transition hover:bg-white/14 hover:text-white sm:min-w-24 sm:flex-row sm:px-4 sm:text-sm"><Icons.logout className="h-5 w-5" /><span>Logout</span></button></div></nav>;
}
